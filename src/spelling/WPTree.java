...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while (!queue.isEmpty()) {
			...
			List<String> currentList = nw.distanceOne(current.getWord(), true);
			for (String n : currentList) {
				if (!visited.contains(n)) {
					WPTreeNode child = current.addChild(n);
					queue.add(child);
					visited.add(n);
					if (n.equals(word2)) return child.buildPathToRoot();
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		List<String> out = tree.findPath("hello", "world");
